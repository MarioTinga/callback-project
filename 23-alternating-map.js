...
//initialize an empty array that will be filled and returned by the end.    
    let alternatingArray = [];
//loop to check each element of the array.    
     for(let i = 0; i < array.length; i++) {
        let el = array[i];
//set a condition to check if the current index is odd or even and then pass in the element in the  first cb and push the result in the newArray
        if(i % 2 === 0){
            alternatingArray.push(cb1(el, i, array));
        }
        else {
//otherwise if the index is the opposite, pass the current element in the secodn callback            
            alternatingArray.push(cb2(el, i, array));
        }
     }
