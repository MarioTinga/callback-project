...
let selectiveMap = function(array, cb1, cb2) {
    ...
    for(let i = 0; i < array.length; i++) {
        let el = array[i];
        //first we try to filter the elements in the array, if the result of the firstcallback is true
        if(cb1(el, i, array)){
        //then we can run the second callback function on the element and push the rusult in a new array    
         selectedElements.push(cb2(el, i, array));
        } else {
        //otherwise, if the rusult of the first cllback is faslse, the element is pushed without any changes    
