...
//we need to initialize a counter that will be checked at the end of the loop    
    let counter = 0;
// a for loop to check each element of the array    
    for(let i = 0 ; i < array.length; i++) {
        let el = array[i];
//setting a condition that increments the counter if the current element returns true when passed in the cb as an argument.
        if(cb(el, i, array)) {
            counter += 1;
        }
    }
// setting a condition that checks if the counter is deeply equal to one, which means that only one element of the array results in true when passed in the cb.
    if(counter === 1) {
        return true;
    }
    return false;
